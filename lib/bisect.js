...
function selectNext(state) {
	...
	return versionsPromise.then(versions => {
		...
		if (possible.length === 0) {
			return `Broken by ${versions[badIdx].semanticVersion}, last good version: ${versions[goodIdx].semanticVersion}`;
		} else {
			...
			console.log(`Looking between bad ${versions[badIdx].semanticVersion} and ${versions[goodIdx].semanticVersion} good versions. Now testing ${useVersion.semanticVersion}.`);
			console.log(`There are ${possible.length} versions left (${possibleLocal.length} already installed), roughly ${Math.ceil(Math.log2(possible.length))} steps left.`);
			if (useVersion.local) {
